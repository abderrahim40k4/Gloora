...
const Product = () => {
  ...
  return (
    ...
    <Carousel
        ...
        className="w-full inline-flex justify-center items-center relative "
    >
        <CarouselContent className="flex gap-8 justify-center items-center relative  mb-6 ">
            {
                prodacts.map((product, index) =>(
                    <CarouselItem  className="lg:basis-1/4 basis-full lg:w-72 relative w-full  h-[26.4rem] rounded-3xl bg-baseColo flex flex-col justify-between items-start p-0 shadow-lg " key={index}  onClick={()=>handleHover(product.selected)}>
